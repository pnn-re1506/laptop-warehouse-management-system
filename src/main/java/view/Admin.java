...
public class Admin extends javax.swing.JFrame {
    ...
    private final Map<String, JInternalFrame> cachedPages = new HashMap<>();
    private String activePageKey = "";
    private boolean pagesPreloaded = false;
...
//        pf.checkRole(currentAcc);
        ...
        showCachedPage("products", ProductForm::new);
        preloadPages();
    }

    private void showCachedPage(String key, Supplier<JInternalFrame> pageFactory) {
        if (key.equals(activePageKey)) {
            return;
        }

        JInternalFrame page = cachedPages.get(key);
        boolean fromCache = page != null && !page.isClosed();
        if (!fromCache) {
            showLoadingPage(key);
            new javax.swing.SwingWorker<JInternalFrame, Void>() {
                @Override
                protected JInternalFrame doInBackground() {
                ...
                protected void done() {
                    try {
                        JInternalFrame loadedPage = get();
                        cachedPages.put(key, loadedPage);
                        if (key.equals(activePageKey)) {
                            showPage(key, loadedPage);
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
        ...
        showPage(key, page);
        if (fromCache) {
            refreshPageAsync(page);
        }
    }

    private void showPage(String key, JInternalFrame page) {
        activePageKey = key;
        MainContent.removeAll();
        page.setBounds(0, 0, MainContent.getWidth(), MainContent.getHeight());
        MainContent.add(page);
        page.setVisible(true);
        MainContent.revalidate();
        MainContent.repaint();
    }

    private void showLoadingPage(String key) {
        activePageKey = key;
        MainContent.removeAll();
        JLabel loading = new JLabel("Loading...", SwingConstants.CENTER);
        loading.setBounds(0, 0, MainContent.getWidth(), MainContent.getHeight());
        MainContent.add(loading);
        MainContent.revalidate();
        MainContent.repaint();
    }

    private String currentUserName() {
        return currentAcc != null ? currentAcc.getUserName() : "";
    }
    ...
    private void preloadPages() {
        ...
        preloadPage("suppliers", SupplierForm::new, 150);
        preloadPage("import", () -> {
            ImportForm im = new ImportForm();
            im.setImportUser(currentUserName());
            return im;
        }, 300);
        preloadPage("receipts", () -> new ReceiptForm(getCurrentAcc()), 450);
        preloadPage("inventory", InventoryForm::new, 600);
        preloadPage("export", () -> {
            ExportForm ex = new ExportForm();
            ex.setCreatedBy(currentUserName());
            return ex;
        }, 750);
        preloadPage("deliveries", () -> new DeliveryForm(getCurrentAcc()), 900);
        preloadPage("accounts", AccountForm::new, 1050);
        preloadPage("statistics", StatisticForm::new, 1200);
    }

    private void preloadPage(String key, Supplier<JInternalFrame> pageFactory, int delayMillis) {
        javax.swing.Timer timer = new javax.swing.Timer(delayMillis, event -> {
            if (!cachedPages.containsKey(key) && !key.equals(activePageKey)) {
                new javax.swing.SwingWorker<JInternalFrame, Void>() {
                    @Override
                    protected JInternalFrame doInBackground() {
                        return pageFactory.get();
    ...
    private void refreshPageAsync(JInternalFrame page) {
        if (page instanceof ProductForm) {
            ((ProductForm) page).refreshDataAsync();
        } else if (page instanceof SupplierForm) {
            ((SupplierForm) page).refreshDataAsync();
        } else if (page instanceof ReceiptForm) {
            ((ReceiptForm) page).refreshDataAsync();
        }
    }
    ...
    private void NhaCungCapMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_NhaCungCapMouseClicked
        // TODO add your handling code here:
        showCachedPage("suppliers", SupplierForm::new);
    ...
    private void TonKhoMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_TonKhoMouseClicked
        // TODO add your handling code here:
        showCachedPage("inventory", InventoryForm::new);
    ...
    private void PhieuNhapMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_PhieuNhapMouseClicked
        // TODO add your handling code here:
        showCachedPage("receipts", () -> new ReceiptForm(getCurrentAcc()));
    ...
    private void PhieuXuatMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_PhieuXuatMouseClicked
        // TODO add your handling code here:
        showCachedPage("deliveries", () -> new DeliveryForm(getCurrentAcc()));
    ...
    private void TaiKhoan1MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_TaiKhoan1MouseClicked
        // TODO add your handling code here:
        showCachedPage("accounts", AccountForm::new);
    ...
    private void ThongKeMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_ThongKeMouseClicked
        // TODO add your handling code here:
        showCachedPage("statistics", StatisticForm::new);
